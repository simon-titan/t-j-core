...
export default function SetPasswordPage() {
  ...
  return (
    <div className="auth-glass-card">
      ...
      {/* ── Loading ── */}
      {status === 'loading' && (
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: '14px', color: 'var(--mute)', textAlign: 'center' }}>
          ...
          <form onSubmit={handleSubmit} noValidate>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
              ...
              <div>
                ...
                {pwTooShort && <p style={hintStyle('#DC2626')}>Mindestens 8 Zeichen erforderlich.</p>}
              </div>
              ...
              <div>
                ...
                {pwMismatch && <p style={hintStyle('#DC2626')}>Passwörter stimmen nicht überein.</p>}
              </div>

              {errorMsg && (
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: '13px', color: '#991B1B', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.18)', borderRadius: 'var(--radius-2)', padding: '10px 14px' }}>
                  {errorMsg}
                </p>
              )}
              ...
              <button
                type="submit"
                disabled={!canSave}
                style={btnStyle(canSave ? '#4A7C5C' : '#C8D5CB')}
              >
...
function hintStyle(color: string): React.CSSProperties {
  return { fontFamily: 'var(--font-sans)', fontSize: '12px', color, marginTop: '5px' };
}
...
function btnStyle(bg: string): React.CSSProperties {
  return {
    ...
    borderRadius: 'var(--radius-2)',
    border: 'none',
    cursor: bg === '#C8D5CB' ? 'not-allowed' : 'pointer',
